  ...
  <section class="v2-ui-preflight">
    <header class="v2-ui-preflight__header">
      <div>
        ...
        <p>{{ statusMessage }}<span v-if="totalSteps"> · {{ completedSteps }} / {{ totalSteps }}</span></p>
      </div>
...
const emit = defineEmits(['retry', 'save-draft', 'adopt'])
...
const statusText = computed(() => ({ queued: '排队中', running: '检查中', passed: '通过', failed: '待修复' }[props.preflight?.status] || '未开始'))
const statusTone = computed(() => ({ passed: 'ok', failed: 'fail', queued: 'warn', running: 'warn' }[props.preflight?.status] || ''))
const statusMessage = computed(() => {
  if (props.preflight?.status === 'passed') return '所有步骤通过，将保存为可执行用例。'

...
- 日本站数据脚本的执行弹窗必须提供客户 ID 输入框；新增脚本不得只依赖数据工厂顶部栏。
- 执行弹窗填写的客户 ID 优先级高于数据工厂顶部栏；弹窗为空时才读取顶部栏客户 ID。
- 客户 ID 支持换行、英文逗号、中文逗号、英文分号和中文分号分隔。
- 多个客户 ID 统一转换为 `customer_ids`，并将第一个 ID 同步为 `customer_id`。
- 前端必须复用公共客户 ID 解析和登录变量处理逻辑，不得在单个脚本中重复实现覆盖规则。
- OEM 脚本不适用日本站顶部栏客户 ID 规则，不能因复用公共执行入口而覆盖 OEM 登录变量。
- 不得因增加客户 ID 输入框修改既有 API 路径、请求结构、脚本类型、注册 key 或执行流程。

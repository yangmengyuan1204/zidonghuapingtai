...
async def _try_login(page: Any, account: str, password: str) -> bool:
    ...
    兼容 Element UI:输入框 .el-input__inner,登录按钮 .el-button--primary(可能是 button 或 a)。
    """
    try:
        # 找密码框(最可靠的登录页标志)
        password_input = await page.query_selector('input[type="password"]')
        if not password_input:
            return False  # 可能已登录或无需登录

        # 找账号框:优先 Element UI 的 .el-input__inner,其次 type 属性
        account_input = await page.query_selector('.el-input__inner, input[type="text"], input[type="tel"], input[type="number"]')
        if not account_input:
            return False

        # Element UI 的 fill 可能需要先聚焦,用 click+type 更稳
        await account_input.click()
        await account_input.fill("")
        await account_input.type(account, delay=50)
        await password_input.click()
        await password_input.fill("")
        await password_input.type(password, delay=50)

        # 找登录按钮:兼容 button/a 标签 + 中英文文字
